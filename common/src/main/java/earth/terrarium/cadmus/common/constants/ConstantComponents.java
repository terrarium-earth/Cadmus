package earth.terrarium.cadmus.common.constants;

import net.minecraft.network.chat.Component;

public class ConstantComponents {

    public static final Component LOADING = Component.translatable("gui.cadmus.claim_map.loading");
    public static final Component CLEAR_DIMENSION = Component.translatable("gui.cadmus.claim_map.clear_dimension");
    public static final Component CLEAR_ALL = Component.translatable("gui.cadmus.claim_map.clear_all");
    public static final Component WILDERNESS = Component.translatable("message.cadmus.wilderness");
}
